 ...
 */
public class Admin extends User implements Reportable, Loggable, Auditable {
    ...
    @Override
    public void recordAudit(String action, long timestamp) {
        String entry = String.format("[%d] %s", timestamp, action);
        if (logCount < auditLogs.length) {
            auditLogs[logCount++] = entry;
        }
    }
    
    @Override
    public String[] getAuditTrail() {
        String[] trail = new String[logCount];
        System.arraycopy(auditLogs, 0, trail, 0, logCount);
        return trail;
    }
}